...
//     return new_h;
// }
// Obtain the new variance array (preallocated new_h), given that the kth observation has been removed
// Refactored after causing problems with an unsigned integer underflow caused when p > 1 or q > 1
std::vector<double> h_remove_k_cpp_call(std::vector<double>& theta, std::vector<double>& y, std::vector<double> h, int k, int p, int q){
    int T = y.size();
    ...
    // If k is the last observation, the variances remain the same
    if(k == T){
        return h;
    }
    ...
    double alpha0 = theta[0];
    std::vector<double> alpha(theta.begin() + 1, theta.begin() + 1 + p);
    std::vector<double> beta(theta.begin() + 1 + p, theta.end());
    
    // Start updating the variance from step k to T-1
    for(int t = k; t < T; t++){
        double arch = alpha0;
        ...
        for (int i = 1; i <= p; i++) {
            int past_index = t - i;
            if (past_index >= 0) {
                // If the index hits the removed observation (k-1 in 0-based), use expected value h_k
                if (past_index == (k - 1)) {
                    arch += alpha[i - 1] * h[k - 1]; // E[y_k^2] = h_k
                } else {
                    arch += alpha[i - 1] * y[past_index] * y[past_index];
                }
            }
        }
        ...
        for (int j = 1; j <= q; j++) {
            int past_index = t - j;
            if (past_index >= 0) {
                // Use new_h for past variances to propagate the new path correctly
                garch += beta[j - 1] * new_h[past_index];
            }
        }
